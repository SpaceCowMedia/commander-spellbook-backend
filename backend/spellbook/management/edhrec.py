...
def edhrec():
    ...
    with urlopen(req) as response:
        ...
        for old_variant_id, variant_data in data['combos'].items():
            if old_variant_id not in variants_id_map:
                continue
            variant_id = variants_id_map[old_variant_id]
        ...
        for old_variant_id in data['errors'].keys():
            if old_variant_id not in variants_id_map:
                continue
            variant_id = variants_id_map[old_variant_id]
            if variant_id not in variants_db:
