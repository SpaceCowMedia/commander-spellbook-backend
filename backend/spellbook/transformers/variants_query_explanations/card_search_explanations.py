...
def card_search_explanation(qv: QueryValue) -> Explanation:
    ...
    match qv.operator:
        case ':' if not value_is_digit:
            return about(qv, USE, Card, f'{ENTITY} with {quoted(qv.value)} in the name')
        case '=' if not value_is_digit:
            return about(qv, USE, Card, f'{ENTITY} named exactly {quoted(qv.value)}')
        case ':' | '=' | '<' | '<=' | '>' | '>=' if value_is_digit:
            return Predicate(USE, count(qv.operator, qv.value, *entity_names(Card)))
        case _:
