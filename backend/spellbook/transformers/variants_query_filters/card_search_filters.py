    ...
    match qv.operator:
        case ':' if not value_is_digit:
            return VariantFilterCollection(cards_filters=(qv.to_query_filter(
                Q(card__name__icontains=qv.value) | Q(card__name_unaccented__icontains=qv.value) | Q(card__name_unaccented_simplified__icontains=qv.value) | Q(card__name_unaccented_simplified_with_spaces__icontains=qv.value)
            ),))
        case '=' if not value_is_digit:
            return VariantFilterCollection(cards_filters=(qv.to_query_filter(
                Q(card__name__iexact=qv.value) | Q(card__name_unaccented__iexact=qv.value) | Q(card__name_unaccented_simplified__iexact=qv.value) | Q(card__name_unaccented_simplified_with_spaces__iexact=qv.value),
            ),))
