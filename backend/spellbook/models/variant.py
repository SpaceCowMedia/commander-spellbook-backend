...
EXACT_SEARCH_FIELDS = Playable.legalities_fields() + ['identity', 'hulkline', 'tutor', 'game_changer', 'reserved', 'mass_land_denial', 'extra_turn']
...
class Variant(Recipe, Playable, Explanation, PreSaveSerializedModelMixin, ScryfallLinkMixin):
    ...
    tutor = models.BooleanField(editable=False, default=False, help_text='Whether the variant uses a card that can tutor for other cards')
    game_changer = models.BooleanField(editable=False, default=False, help_text='Whether the variant uses a card in the official Game Changer card list')
    reserved = models.BooleanField(editable=False, default=False, help_text='Whether the variant uses a card on the Reserved List')
    mass_land_denial = models.BooleanField(editable=False, default=False, help_text='Whether the variant produces mass land denial')
    extra_turn = models.BooleanField(editable=False, default=False, help_text='Whether the variant produces extra turns for its controller')
    ...
    def update_variant_from_recipe(
        ...
        self.tutor = any(card.tutor for _, card in recipe.cards)
        self.game_changer = any(card.game_changer for _, card in recipe.cards)
        self.reserved = any(card.reserved for _, card in recipe.cards)
        self.mass_land_denial = any(MASS_LAND_DENIAL_PATTERN.search(feature.name) for _, feature in recipe.features)
        self.extra_turn = any(
        ...
        # read back by estimate_bracket below, which is why they are computed before it runs
...
def estimate_bracket(cards: dict[Card, int], commanders: set[Card] | None, included_variants: Sequence[tuple[Variant, Variant.Recipe]]) -> BracketEstimate:
    '''The bracket a list of cards belongs to, out of what those cards and the combos they make are.'''
    combos: list[ClassifiedCombo] = []
    for variant, recipe in included_variants:
        ...
        combos.append(ClassifiedCombo(
            ...
            speed=speed,
            mass_land_denial=variant.mass_land_denial,
            extra_turn=variant.extra_turn,
            lock=lock,
